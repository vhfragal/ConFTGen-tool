...
public class SGraphJavaValidator extends AbstractDeclarativeValidator {
	...
	public static final String ISSUE_INVALID_CONDITIONAL_LOOP_STATE = "Invalid self-loop feature Constraint:";
	...
	*/
	...
	public ArrayList<FState> get_equivs(org.yakindu.sct.model.sgraph.State state){
		//FState equiv = null;
		ArrayList<FState> equivs = new ArrayList<FState>();
		try {
			String s = getZ3_state_name(state);
			String[] pair = s.split("@");			
			for(FState fs1 : p.getFFSM().getFStates()){
				//simple find
				if(pair[0].equals(fs1.getStateName())){
					if(!equivs.contains(fs1)) equivs.add(fs1);
				}
				//composed states
				String[] composed = null;
				if(fs1.getStateName().indexOf("*") > 1){
					composed = fs1.getStateName().split("\\*");
				}//else if(fs1.getStateName().indexOf(":") > 1){
				//	composed = fs1.getStateName().split(":");
				//}
				if(composed != null){
					for(String st : composed){
						if(st.equals(pair[0])){
							if(!equivs.contains(fs1)) equivs.add(fs1);
						}
					}
	...
	public FState get_equivalent(org.yakindu.sct.model.sgraph.State state){
		...
		try {
			...
			String[] pair = s.split("@");
			for(FState fs : p.getFFSM().getFStates()){							
				if(pair[0].equals(fs.getStateName()) && p.check_equiv_cond(fs.getCondition(),pair[1])){					
					return fs;
				}	
			}
			//composed state			
	...
	public void check_state_fcondition_onSave(org.yakindu.sct.model.sgraph.State state) {				
		...
		if(root == state){
		    ...
		    System.out.println("FFSM Flat gen. (secs) "+elapsedTime/1000);
			...
			for(FState f : not_init_con){
				ArrayList<org.yakindu.sct.model.sgraph.State> list = get_RootStateList(root);
				//list.addAll(get_StateList(root));				
				System.out.println("NOT INITI CONN "+ list);
				for(org.yakindu.sct.model.sgraph.State st : list){					
					//FState q = get_equivalent(st);
					ArrayList<FState> equivs = get_equivs(st);
					for(FState equiv : equivs){
						if(f.equals(equiv)){
		...
		//System.out.println("AFTER MINIMAL ---"+state.getName());		
		...
		//System.out.println("AFTER STATE COND ---"+state.getName());
		...
		//System.out.println("AFTER REGION COND ---"+state.getName());
		String condition = get_state_hierarchical_condition(state);
		if(condition.length() > 0){
			error(ISSUE_INVALID_HCONDITIONAL_STATE+condition, state, null, -1);
		}
		
		//check inner transition constraint				
		String spec = state.getSpecification();
		if(spec != null){
			String[] trs = spec.split("\n");		
			for(String s : trs){
				if(s.indexOf("[") > 0 && s.indexOf("]") > s.indexOf("[")){
					String icond = get_condition(s);
					if(icond.length() > 0){					
						error(ISSUE_INVALID_CONDITIONAL_LOOP_STATE+icond, state, null, -1);
					}
				}
			}
		}
		...
		for(Transition t : state.getOutgoingTransitions()){
			...
			if(spec != null){
				...
				condition = get_transition_condition(t);
				if(condition.length() > 0){
		...
		//check determinism	
		ArrayList<FState> equivs = get_equivs(state);
		for(FState equiv : equivs){
		...
		//ArrayList<FState> list = (ArrayList<FState>) p.getFFSM().getFStates().clone();
		for(FState equiv : equivs){
			//list.remove(equiv);
			for(FState fs : p.getFFSM().getFStates()){
				if(!fs.equals(equiv)){	
	...
	//@Check(CheckType.EXPENSIVE)
	public void check_state_fcondition_slow(org.yakindu.sct.model.sgraph.State state) {				
		Statechart sct = get_Statechart(state);
		org.yakindu.sct.model.sgraph.State root = get_StatechartRoot(state); 
		//System.out.println("CHECK STATE ---"+state.getName());
		if(root == state){
		...
		//System.out.println("GET EQUIV ---"+state.getName());
		ArrayList<FState> equivs = get_equivs(state);
		//System.out.println("AVALI STATE "+equivs+" "+state.getName()+" "+p.getFFSM().getFStates());
	...
	public ArrayList<org.yakindu.sct.model.sgraph.State> get_RootStateList(org.yakindu.sct.model.sgraph.State root){
		ArrayList<org.yakindu.sct.model.sgraph.State> list = new ArrayList<org.yakindu.sct.model.sgraph.State>();		
			for(EObject state : root.getParentRegion().eContents()){				
				try{
	...
	public org.yakindu.sct.model.sgraph.State get_StatechartRoot(org.yakindu.sct.model.sgraph.State s){		
		...
		for(int i=0; i < 100; i++){
			try{
				Statechart sct = (Statechart) source.getParentRegion().getComposite();
				break;
			}catch (Exception e){
				source = (org.yakindu.sct.model.sgraph.State) source.getParentRegion().getComposite();
			}
